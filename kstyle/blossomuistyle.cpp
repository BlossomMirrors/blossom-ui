...
#endif
{
    ...
    dbus.connect(QString(),
                 QStringLiteral("/BlossomUIDecoration"),
                 QStringLiteral("org.kde.BlossomUI.Style"),
                 QStringLiteral("reparseConfiguration"),
                 this,
                 SLOT(configurationChanged()));
...
void Style::polish(QWidget *widget)
{
    ...
    if (_isDolphin && widget->inherits("DolphinView")) {
...
bool Style::eventFilter(QObject *object, QEvent *event)
{
    ...
    } else if (_isDolphin && qobject_cast<QListView *>(object)) {
        QListView *listView = static_cast<QListView *>(object);
            ...
            if (widget->isWindow() && !_isKonsole && widget->testAttribute(Qt::WA_StyledBackground) && widget->testAttribute(Qt::WA_TranslucentBackground)) {
                ...
                case Qt::Sheet: {
                    ...
                    /*if( (widget->palette().color( QPalette::Window ).alpha()/255)*100  < _helper->titleBarColor( true ).alphaF()*100.0 )
                    {
                        ...
                        {
                            QSet<const QWidget *>::const_iterator i = BlossomUIPrivate::possibleTranslucentToolBars.constBegin();
    ...
    if (event->type() == QEvent::Paint) {
        ...
        if (dockWidget->isFloating()) {
            ...
            if (_isDolphin && dockWidget->inherits("DolphinDockWidget") && _translucentWidgets.contains(dockWidget->window())) {
                ...
                // side shadow
                if (StyleConfigData::dolphinSidebarOpacity() < (palette.color(QPalette::Window).alpha() / 255.0) * 100) {
                    /*painter.setBrush( Qt::NoBrush );
...
bool Style::eventFilterCommandLinkButton(QCommandLinkButton *button, QEvent *event)
{
    if (event->type() == QEvent::Paint) {
        ...
        if (!button->icon().isNull()) {
            ...
            const QRect pixmapRect(QPoint(offset.x(), button->description().isEmpty() ? qRound((button->height() - pixmapSize.height()) / 2.0) : offset.y()),
                                   pixmapSize);
...
QRect Style::progressBarGrooveRect(const QStyleOption *option, const QWidget *widget) const
{
    ...
    const bool horizontal(BlossomUIPrivate::isProgressBarHorizontal(progressBarOption));
...
QRect Style::tabWidgetTabBarRect(const QStyleOption *option, const QWidget *widget) const
{
    ...
    if (verticalTabs) {
        ...
        if (tabBarAlignment == Qt::AlignCenter)
            tabBarRect.moveTop(rect.top() + (rect.height() - tabBarRect.height()) / 2.0);
        else
        ...
        if (tabBarAlignment == Qt::AlignCenter) {
            tabBarRect.moveLeft(rect.left() + (rect.width() - tabBarRect.width()) / 2.0);
...
QRect Style::dialSubControlRect(const QStyleOptionComplex *option, SubControl subControl, const QWidget *widget) const
{
    ...
    case QStyle::SC_DialGroove:
        return insideMargin(rect, static_cast<int>(Metrics::Slider_ControlThickness - Metrics::Slider_GrooveThickness) / 2.0);
    case QStyle::SC_DialHandle: {
        ...
        // groove rect
        const QRectF grooveRect(insideMargin(rect, static_cast<int>(Metrics::Slider_ControlThickness) / 2.0));
...
bool Style::drawFramePrimitive(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (!StyleConfigData::sidePanelDrawFrame() && widget && widget->property(PropertyNames::sidePanelView).toBool()) {
        ...
        /*if( _frameShadowFactory->isRegistered( widget ) ) // WHAT does this do?? // Dude i often have this with this weird ass codebase
        {
...
bool Style::drawPushButtonLabelControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (hasText && !hasIcon)
        ...
        iconRect = QRect(QPoint(contentsRect.left() + (contentsRect.width() - contentsWidth) / 2,
                                contentsRect.top() + qRound((contentsRect.height() - iconSize.height()) / 2.0)),
                         iconSize);
        textRect = QRect(
            QPoint(iconRect.right() + Metrics::ToolButton_ItemSpacing + 1, contentsRect.top() + qRound((contentsRect.height() - textSize.height()) / 2.0)),
            textSize);
    }
...
bool Style::drawToolButtonLabelControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (hasText && (!(hasArrow || hasIcon) || toolButtonOption->toolButtonStyle == Qt::ToolButtonTextOnly)) {
        ...
        iconRect = QRect(QPoint(contentsRect.left() + qRound((contentsRect.width() - iconSize.width()) / 2.0),
                                contentsRect.top() + qRound((contentsRect.height() - contentsHeight) / 2.0)),
                         iconSize);
        textRect = QRect(
            QPoint(contentsRect.left() + qRound((contentsRect.width() - textSize.width()) / 2.0), iconRect.bottom() + Metrics::ToolButton_ItemSpacing + 1),
            textSize);
        textFlags |= Qt::AlignCenter;
        ...
        if (leftAlign) {
            ...
            iconRect = QRect(QPoint(contentsRect.left() + qRound((contentsRect.width() - contentsWidth) / 2.0),
                                    contentsRect.top() + qRound((contentsRect.height() - iconSize.height()) / 2.0)),
                             iconSize);
        }
...
bool Style::drawTabBarTabShapeControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const
{
    ...
    QColor backgroundColor;
    if (StyleConfigData::documentModeTabs())
        backgroundColor = documentMode ? _helper->isDarkTheme(palette) ? _helper->alphaColor(configTabBgColor, 0.4) : _helper->alphaColor(configTabBgColor, 0.2)
...
bool Style::drawSliderComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (sliderOption->subControls & SC_SliderGroove) {
        ...
        // base color
        const auto grooveColor(_helper->alphaColor(palette.color(QPalette::WindowText), 0.16));
        ...
        if (!enabled)
            ...
            // highlight color
            const auto highlight(KColorUtils::mix(palette.color(QPalette::Highlight), palette.color(QPalette::Window), 0.25));
...
bool Style::drawDialComplexControl(const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    if (sliderOption->subControls & SC_DialGroove) {
        ...
        // groove
        const auto grooveColor(KColorUtils::mix(palette.color(QPalette::Window), palette.color(QPalette::WindowText), 0.16));
        ...
        if (tb->orientation() == Qt::Vertical) {
            if (tb->y() == 0) {
                if (BlossomUIPrivate::possibleTranslucentToolBars.size() == 0) {
                    ...
                    return true;
                } else if (BlossomUIPrivate::possibleTranslucentToolBars.contains(w) && BlossomUIPrivate::possibleTranslucentToolBars.size() == 1)
                    return true;
