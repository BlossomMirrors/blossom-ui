...
bool Render::HeaderControl::drawHeaderSectionControl(const QStyleOption *option,
                                     QPainter *painter,
  ...
  _style->_animations->headerViewEngine().updateState(widget, rect.topLeft(),
                                              mouseOver);
  const bool animated(enabled && _style->_animations->headerViewEngine().isAnimated(
                                     widget, rect.topLeft()));
  const qreal opacity(
      _style->_animations->headerViewEngine().opacity(widget, rect.topLeft()));

  const QColor color = Render::headerSectionFill(_style->_helper, palette, sunken,
                                                 mouseOver, animated, opacity)
                           .brush.color();
  ...
  if (corners) {
    _style->_helper->renderSelection(painter, rect, color, corners, radius);
  } else {
  ...
  painter->setPen(
      _style->_helper->alphaColor(palette.color(QPalette::WindowText), 0.1));
  ...
  } else if (horizontal && !_style->_app.isDolphin) {
  ...
  // separators
  painter->setPen(
      _style->_helper->alphaColor(palette.color(QPalette::WindowText), 0.2));
...
bool Render::HeaderControl::drawHeaderEmptyAreaControl(const QStyleOption *option,
                                       QPainter *painter,
                                       const QWidget *) const {
  ...
  painter->setBrush(Qt::NoBrush);
  painter->setPen(
      _style->_helper->alphaColor(palette.color(QPalette::ButtonText), 0.1));
...
bool Style::drawHeaderSectionControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const {
  return Render::HeaderControl(this).drawHeaderSectionControl(option, painter, widget);
}

bool Style::drawHeaderEmptyAreaControl(const QStyleOption *option, QPainter *painter, const QWidget * a2) const {
  return Render::HeaderControl(this).drawHeaderEmptyAreaControl(option, painter, a2);
}
} // namespace BlossomUI
