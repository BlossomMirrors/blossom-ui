...
bool Render::ToolBarControl::drawToolBarBackgroundControl(const QStyleOption *option,
                                         QPainter *painter,
  ...
  if (_style->_app.isDolphin && StyleConfigData::dolphinSidebarOpacity() < 100 &&
      !(option->state & QStyle::State_Horizontal)) {
    sideToolbarDolphin = true;
  }

  else if (!_style->isStylableToolbar(widget)) {
    return true;
  }
  ...
  if (StyleConfigData::toolBarOpacity() < 100 && !_style->_app.isOpaque) {
  ...
  if (sideToolbarDolphin && _style->_app.isDolphin) {
    backgroundColor.setAlphaF(StyleConfigData::dolphinSidebarOpacity() / 100.0 -
                              0.15);
    painter->fillRect(rect, backgroundColor);

    bool darkTheme(_style->_helper->isDarkTheme(palette));
  ...
  if (StyleConfigData::toolBarDrawSeparator() && !_style->_app.isDolphin) {
    painter->setBrush(Qt::NoBrush);
      ...
      if (StyleConfigData::widgetToolBarShadow()) {
        ...
        if (!darkTheme) {
          ...
          QRect shadowRect(copy.bottomLeft() + QPoint(-1, 1),
                           QSize(copy.width(), 50));
          _style->_helper->renderBoxShadow(painter, shadowRect, 0, 0, 8,
                                   QColor(0, 0, 0, 160), 2, true);
          _style->_helper->renderBoxShadow(painter, shadowRect, 0, 0, 3,
                                   QColor(0, 0, 0, 160), 2, true);
        }
      }
...
bool Style::drawToolBarBackgroundControl(const QStyleOption *option, QPainter *painter, const QWidget *widget) const {
  return Render::ToolBarControl(this).drawToolBarBackgroundControl(option, painter, widget);
}
} // namespace BlossomUI
