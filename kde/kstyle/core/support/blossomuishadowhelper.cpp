...
namespace
{
using BlossomUI::CompositeShadowParams;
using BlossomUI::ShadowParams;
...
TileSet ShadowHelper::shadowTiles(QWidget *widget)
{
    ...
    painter.drawRoundedRect(outerRect - margins, frameRadius - 1, frameRadius - 1);
...
TileSet ShadowHelper::shadowTiles(const int frameRadius, CustomShadowParams shadow1, CustomShadowParams shadow2)
{
    ...
    const QPoint innerRectTopLeft = outerRect.center();
    TileSet shadowTiles = TileSet(QPixmap::fromImage(shadowTexture), innerRectTopLeft.x(), innerRectTopLeft.y(), 1, 1);
