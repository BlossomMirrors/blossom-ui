  ...
  if (foregroundColor.isValid()) {
    ...
    case DecorationButtonType::Maximize:
      if (isChecked()) {
        ...
        painter->setPen(pen);
        painter->drawPolygon(QVector<QPointF>{QPointF(4, 9), QPointF(9, 4),
                                              QPointF(14, 9), QPointF(9, 14)});
      } else {
    ...
    case DecorationButtonType::OnAllDesktops:
      ...
      if (isChecked()) {
        ...
        painter->drawPolygon(QVector<QPointF>{QPointF(6.5, 8.5), QPointF(12, 3),
                                              QPointF(15, 6),
                                              QPointF(9.5, 11.5)});
    ...
    case DecorationButtonType::Shade:
      if (isChecked()) {
        ...
        painter->drawPolyline(
            QVector<QPointF>{QPointF(4, 8), QPointF(9, 13), QPointF(14, 8)});
      } else {
        painter->drawLine(QPointF(4, 5.5), QPointF(14, 5.5));
        painter->drawPolyline(
            QVector<QPointF>{QPointF(4, 13), QPointF(9, 8), QPointF(14, 13)});
      }
      break;

    case DecorationButtonType::KeepBelow:
      painter->drawPolyline(
          QVector<QPointF>{QPointF(4, 5), QPointF(9, 10), QPointF(14, 5)});
      painter->drawPolyline(
          QVector<QPointF>{QPointF(4, 9), QPointF(9, 14), QPointF(14, 9)});
      break;

    case DecorationButtonType::KeepAbove:
      painter->drawPolyline(
          QVector<QPointF>{QPointF(4, 9), QPointF(9, 4), QPointF(14, 9)});
      painter->drawPolyline(
          QVector<QPointF>{QPointF(4, 13), QPointF(9, 8), QPointF(14, 13)});
      break;
...
void Button::drawIconWithMask(QPainter *painter) const {
  ...
  case DecorationButtonType::Close:
    ...
    break;
  case DecorationButtonType::Maximize:
    iconName = isChecked() ? QStringLiteral("window-restore-symbolic")
  ...
  const QString iconPath =
      KIconLoader::global()->iconPath(iconName, KIconLoader::Small, true);
  if (iconPath.isEmpty()) {
    drawIcon(painter);
    return;
  }
  ...
  int iconWidth = qRound(drawRect.width()) - 2;
  int iconHeight = qRound(drawRect.height()) - 2;
  ...
  const QPoint iconPos(
      qRound(rect.x() + rect.width() / 2.0 - iconWidth / 2.0),
      qRound(rect.y() + rect.height() / 2.0 - iconHeight / 2.0));
