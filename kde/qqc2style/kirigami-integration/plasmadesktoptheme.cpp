...
void PlasmaDesktopTheme::syncColors()
{
    ...
    // Blossom uses translucent selection backgrounds (see the qqc2style
    // delegates and the kstyle's drawPanelItemViewItemPrimitive), so
    // highlighted text keeps the normal foreground instead of the selection
    // foreground, which would be unreadable on a translucent highlight.
    setHighlightedTextColor(colors.scheme.foreground(KColorScheme::NormalText).color());
