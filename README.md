...
This repository is the home of all design elements of Blossom / BlossomOS.

- Complete KDE Plasma desktop design / QT Themes
- [Shadcn-Svelte Registry](https://shadcn-svelte.com/docs/registry)
- Colors, fonts, logos...

## 1. Color System

All palette token values (neutral, primary, secondary, tertiary, quaternary) live in [`colors.json`](./colors.json). That file is the single source of truth. In `desktop/kde/colors` the `.colors` schemes are generated from it at build time via `cmake/BlossomUIColors.cmake`. The shadcn-svelte registry extracts its colors from it too.
