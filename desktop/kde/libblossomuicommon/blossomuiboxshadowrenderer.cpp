...
static inline void boxBlurAlpha(QImage &image, int radius,
  ...
  for (int i = 0; i < height; ++i) {
    uint8_t *row = image.scanLine(blurRect.y() + i) +
                   blurRect.x() * pixelStride + alphaOffset;
  ...
  for (int i = 0; i < width; ++i) {
    uint8_t *column = image.scanLine(blurRect.y()) +
                      (blurRect.x() + i) * pixelStride + alphaOffset;
...
static void renderShadow(QPainter *painter, const QRectF &rect,
                         qreal borderRadius, const QPointF &offset,
  ...
  // For some reason, if radius is <=3, the shadow edge becomes too sharp, so we
  // work around this. FIXME shadowPainter.drawRoundedRect(boxRect, xRadius,
  // yRadius);
  shadowPainter.drawRoundedRect(boxRect, radius > 3 ? xRadius : borderRadius,
                                radius > 3 ? yRadius : borderRadius);
  shadowPainter.end();

  // Because the shadow texture is symmetrical, that's enough to blur
  // only the top-left quadrant and then mirror it.
  const QRect blurRect(0, 0, std::ceil(shadow.width() * 0.5),
                       std::ceil(shadow.height() * 0.5));
...
void BoxShadowRenderer::setBoxSize(const QSizeF &size) { m_boxSize = size; }

void BoxShadowRenderer::setBorderRadius(qreal radius) {
  m_borderRadius = radius;
}

void BoxShadowRenderer::addShadow(const QPointF &offset, double radius,
                                  const QColor &color) {
  Shadow shadow = {};
...
QSizeF BoxShadowRenderer::calculateMinimumShadowTextureSize(
    const QSizeF &boxSize, double radius, const QPointF &offset) {
  return boxSize + 2 * calculateBlurExtent(radius) +
         QSizeF(std::abs(offset.x()), std::abs(offset.y()));
}
