...
bool Style::drawTabBarTabLabelControl(const QStyleOption *option,
  ...
  if (tabOption) {
    ...
    tr = subElementRect(SE_TabBarTabText, option, widget);

    const bool isSelected = tabOption->state & State_Selected;
    const bool isEnabled = tabOption->state & State_Enabled;

    if (!tabOption->icon.isNull()) {
      QSize iconSize = tabOption->iconSize;
      if (!iconSize.isValid()) {
        const int e = pixelMetric(QStyle::PM_SmallIconSize, option, widget);
        iconSize = QSize(e, e);
      }
      ...
      proxy()->drawItemPixmap(painter, drawRect, Qt::AlignCenter, tabIcon);
    ...
    proxy()->drawItemText(painter, tr, alignment, tabOption->palette, isEnabled,
...
bool Style::drawTabBarTabShapeControl(const QStyleOption *option,
  ...
  const bool isDark = _helper->isDarkTheme(palette);
  ...
  if (selected) {
    ...
    if (animated)
      color = KColorUtils::mix(transparent, hover, opacity);
    else if (mouseOver)
      color = hover;
    else
      color = transparent;
  ...
  if (StyleConfigData::tabBarAltStyle()) {
    ...
    const QColor tabOutlineColor(_helper->isDarkTheme(palette)
                                     ? _helper->alphaColor(Qt::white, 0.10)
    ...
    if (documentMode) {
      ...
      if (color.alphaF() >= 1) {
        _helper->renderTabBarTabOutline(painter, backgroundRect,
